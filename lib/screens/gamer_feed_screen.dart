...
class _GamerFeedScreenState extends State<GamerFeedScreen> {
  ...
  Future<String?> _getUid() async {
    ...
    return (uid != null && uid.isNotEmpty) ? uid : 'test_user_123';
  }
    ...
    try {
      ...
      try {
        if (currentUserId != null) {
          final myLikes = await SupabaseService.client
              .from('likes')
              .select('post_id')
              .eq('user_id', currentUserId!);
          likedPostIds = (myLikes as List)
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Feed Error: $e')),
        );
      }
    }
  }
  ...
  Future<void> _handleLike(Map<String, dynamic> post) async {
    if (currentUserId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Login ID null hai')),
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Liked (count only)')),
          );
        }
      } catch (e2) {
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    final shortUid = currentUserId == null
        ? 'null'
        : (currentUserId!.length > 6
            ? currentUserId!.substring(0, 6)
            : currentUserId!);
            ...
            : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final username = users?['username'] ??
                      ...
                      post['username'] ??
                      'Unknown';
                  final avatarUrl = (users?['avatar_url'] ?? post['user_avatar'] ?? '').toString();
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            Expanded(
                              child: InkWell(
                                onTap: () => _showComments(post['id'].toString()),
                                child: Padding(
                                  padding: const EdgeInsets.symmetric(vertical: 12),
                                  child: Row(
                                    mainAxisAlignment: MainAxisAlignment.center,
  ...
  Future<void> _addComment() async {
    ...
    try {
      await SupabaseService.client.from('comments').insert({
        ...
        'user_id': widget.currentUserId ?? 'test_user',
        'content': text,
        'username': 'You',
      });
