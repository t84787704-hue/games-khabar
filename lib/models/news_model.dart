...
class NewsModel {
  ...
  static Map<String, String> _createDefaultMap(String text) {
    return {
      'roman': text, 'ro': text, 'en': text, 'hi': text,
      'ur': text, 'bn': text, 'ar': text, 'zh': text, 'zh-cn': text,
    };
  }
  ...
  String getTitle([String? langCode]) {
    if (langCode!= null && langCode.isNotEmpty) {
      ...
      if (titleMap.containsKey(code) && titleMap[code]!.trim().isNotEmpty) return titleMap[code]!;
    }
    return titleMap['roman']?? titleMap['en']?? (titleMap.values.isNotEmpty? titleMap.values.first : '');
  }

  String getDescription([String? langCode]) {
    if (langCode!= null && langCode.isNotEmpty) {
      final code = langCode.toLowerCase();
      if (descriptionMap.containsKey(code) && descriptionMap[code]!.trim().isNotEmpty) return descriptionMap[code]!;
    }
    return descriptionMap['roman']?? descriptionMap['en']?? (descriptionMap.values.isNotEmpty? descriptionMap.values.first : '');
  }
  ...
  factory NewsModel.fromFirestore(DocumentSnapshot doc) {
    ...
    if (ts!= null) {
      ...
      if (diff.inMinutes < 60) formattedTime = '${diff.inMinutes <= 0? 1 : diff.inMinutes}m ago';
      else if (diff.inHours < 24) formattedTime = '${diff.inHours}h ago';
      else formattedTime = '${diff.inDays}d ago';
    }
    ...
    Map<String, String> parseTextMap(dynamic val, String fallback) {
      if (val is Map) {
        ...
        return res;
      } else if (val is String && val.isNotEmpty) return _createDefaultMap(val);
  ...
  NewsModel copyWith({ String? id, String? title, Map<String, String>? titleMap, String? description, Map<String, String>? descriptionMap, String? category, String? imageUrl, String? videoUrl, String? youtubeId, String? timeAgo, int? views, double? rating, String? originalPrice, bool? isFree, bool? isFeatured, String? timeLeft, String? storeName, String? downloadSize, String? sourceUrl, bool? isAuto, Timestamp? timestamp, int? appId}) {
    return NewsModel(
      id: id?? this.id,
  ...
  Map<String, dynamic> toJson() {
    return {
      'id': id, 'title': titleMap, 'content': descriptionMap, 'description': descriptionMap,
      'category': category, 'imageUrl': imageUrl, 'videoUrl': videoUrl, 'youtubeId': youtubeId,
      'timeAgo': timeAgo, 'views': views, 'rating': rating, 'originalPrice': originalPrice,
      'isFree': isFree, 'isFeatured': isFeatured, 'timeLeft': timeLeft, 'storeName': storeName,
      'downloadSize': downloadSize, 'sourceUrl': sourceUrl, 'isAuto': isAuto, 'appId': appId,
  ...
  factory NewsModel.fromJson(Map<String, dynamic> json) {
    Timestamp? ts;
    if (json['timestampMillis']!= null) ts = Timestamp.fromMillisecondsSinceEpoch(json['timestampMillis'] as int);
    Map<String, String> parseTextMap(dynamic val, String fallback) {
      if (val is Map) {
        final res = <String, String>{};
        val.forEach((k, v) { if (v!= null) res[k.toString()] = v.toString(); });
    ...
    if (finalImageUrl.isEmpty) finalImageUrl = 'https://picsum.photos/seed/${json['id']}/800/600';
    return NewsModel(
